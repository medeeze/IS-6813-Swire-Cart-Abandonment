...
## Business Problem
MyCoke360 experiences a high rate of cart abandonment where customers add products but fail to complete the purchase before their order window closes. The objective of this project is to analyze customer behavior within each order window to understand what actions or patterns lead to abandonment and why customers fail to convert. By identifying the behavioral patterns associated with the drop off, we can deliver actionable recommendations that would help Swire reduce lost orders and improve the platform’s overall conversion performance. As a result of lowering abandonment rate, Swire captures more of the potential revenue that would otherwise be lost.
